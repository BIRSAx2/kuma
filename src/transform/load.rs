...
//!
//! Finds reaching stores and inserts phi nodes where forwarded values merge.

use crate::analysis::alias::{alias, escapes};
use crate::analysis::control_flow::dom;
use crate::ir::builder::{getcon, newcon, newtmp};
use crate::ir::internal::{AliasType, BlkId, Cls, Con, ConType, Fn, Ins, Op, Phi, Ref, TmpId};

/// Bitmask for a width in bytes: `(1 << (8*w - 1)) * 2 - 1`.
/// Must work when w == 8.
#[inline]
fn mask_for(w: i32) -> u64 {
    if w == 8 { !0u64 } else { (1u64 << (8 * w)) - 1 }
}
...
impl<'a> LoadOpt<'a> {
            ...
            match a.typ {
                ...
                AliasType::Con | AliasType::Sym => {
                    let c = Con {
                        typ: ConType::Addr,
                        sym: a.sym,
                        bits: crate::ir::internal::ConBits::from_i64(a.offset),
                        ..Con::default()
                    };
                    r = newcon(&c, self.f);
                }
    ...
    fn def(&mut self, sl: &Slice, msk: u64, bid: BlkId, i_start: Option<usize>, il: &Loc) -> Ref {
        ...
        while i_idx > 0 {
            ...
            match alias_result {
                crate::ir::internal::AliasResult::MustAlias => {
                    ...
                    if off != 0 {
                        let mut cls1 = cls;
                        if shift_op == Op::Shr && off + sl.sz as i32 > 4 {
                            cls1 = Cls::Kl;
                        }
                ...
                crate::ir::internal::AliasResult::MayAlias => {
                    if ld {
                        continue;
                    }
                    self.f.tmps.truncate(old_ntmp);
                ...
                crate::ir::internal::AliasResult::NoAlias => {
                    continue;
                }
            }
        }
...
/// Eliminate redundant loads using alias analysis.
///
/// Requires RPO, SSA, and alias information.
