//! Stack-slot promotion and coalescing.

use crate::analysis::alias::getalias;
use crate::ir::internal::OP_TABLE;
use crate::ir::internal::{AliasType, BlkId, Cls, Fn, Ins, Jmp, N_BIT, Op, Ref, TmpId};
use crate::transform::load::{loadsz, storesz};

/// Promote uniformly accessed stack slots to SSA temporaries.
///
/// Requires use-def chains and preserves use counts.
pub fn promote(f: &mut Fn) {
    ...
    for (ins_idx, i) in start_ins.iter().enumerate() {
        ...
        for u in &t.uses {
            match u.typ {
                crate::ir::internal::UseType::Ins => {
                    let use_blk = &f.blks[u.bid as usize];
                    let use_ins = match u.detail {
                        crate::ir::internal::UseDetail::InsIdx(idx) => {
        ...
        for u in &uses {
            if u.typ != crate::ir::internal::UseType::Ins {
                continue;
            }
            let use_idx = match u.detail {
                crate::ir::internal::UseDetail::InsIdx(idx) => idx as usize,
                _ => continue,
            };
...
fn slot_store(r: Ref, x: u64, store: Store, f: &Fn, slots: &mut [Slot], slot_map: &[i32]) {
    if let Some((si, off)) = resolve_slot(r, f, slot_map) {
        let s = &mut slots[si];
        if s.l != 0 {
            s.r.add(store.ip);
...
/// Eliminate dead stores and merge stack slots with disjoint live ranges.
pub fn coalesce(f: &mut Fn) {
    ...
    for (n, slot_entry) in slot_map.iter_mut().enumerate().take(ntmp) {
        let t = &f.tmps[n];
...

    {
        for ni in 0..nblk {
            ...
            for &p in &preds {
                ...
                if p_id >= ni as u32 && blk_loop[bid.0 as usize] < p_id as i32 {
    ...
    for n in (0..nblk).rev() {
        ...
        for s in &mut slots {
            s.l = 0;
            for succ_id in succ.iter().flatten() {
            ...
            match u.typ {
                ...
                crate::ir::internal::UseType::Ins => {
                    let idx = match u.detail {
                        crate::ir::internal::UseDetail::InsIdx(i) => i as usize,
                        _ => continue,
                    };
            ...
            if r.overlaps(live_slots[si].r) {
                for mi in n..si {
                    if live_slots[mi].fused == Some(n)
                        && live_slots[mi].r.overlaps(live_slots[si].r)
                    {
                        can_fuse = false;
                        break;
                    }
