...
//!
//! Computes block liveness and peak register pressure in reverse RPO.

use crate::ir::internal::{BSet, Fn, Op, Ref, Target};
        ...
        for a in 0..narg {
            ...
            if blk_id.0 == b_id {
                ...
                if let Ref::Tmp(tid) = arg {
                    v.set(tid.0);
                    f.blks[b_id as usize].live_gen.set(tid.0);
                }
            }
        }
    }
...
/// Mark a reference as live and update its register-class count.
#[inline]
fn bset_ref(r: Ref, b_id: u32, nlv: &mut [i32; 2], f: &mut Fn) {
    if let Ref::Tmp(tid) = r {
        let t = tid.0;
        f.blks[b_id as usize].live_gen.set(t);
...
/// Compute block liveness and peak integer and floating-point pressure.
///
/// Requires reverse-postorder block numbering.
...
fn filllive_inner(f: &mut Fn, t: Option<&Target>) {
    ...
    while chg {
        ...
        for n in (0..nblk).rev() {
            ...
            if let Some(t) = t
                && !f.blks[b_idx].out.bits_raw().is_empty()
            {
            ...
            for i in (0..nins).rev() {
                ...
                for (k, &live) in nlv.iter().enumerate() {
                    if live > f.blks[b_idx].nlive[k] {
                        f.blks[b_idx].nlive[k] = live;
                    }
                }
            }
        }
