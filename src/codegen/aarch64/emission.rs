...
fn loadcon(c: &Con, r: u32, k: Cls, e: &mut E) {
    ...
    if (n | 0xffff) == -1 || logimm(n as u64, k) {
        ...
        let bits = if w { n as u64 } else { (n as u32) as u64 };
    ...
    match i.op {
        ...
        Op::Copy => {
            ...
            if let Ref::Slot(_) = i.to {
                ...
                let store_op = match i.cls {
                    Cls::Kw => Op::Storew,
                    Cls::Kl => Op::Storel,
                    Cls::Ks => Op::Stores,
                    Cls::Kd => Op::Stored,
                    Cls::Kx => unreachable!("copy to stack slot without a value class"),
                };
                let store_ins = Ins {
                    op: store_op,
                    cls: Cls::Kw,
...
fn emit_from_table(i: &Ins, e: &mut E) {
    ...
    for entry in OMAP {
        if entry.op == i.op
            && (entry.cls == k || entry.cls == KA || (entry.cls == KI && i.cls.base() == 0))
        {
            emitf(entry.asm, i, e);
            return;
        }
...
pub fn emitfn(
        ...
        match blk.jmp.typ {
                ...
                for &r in ARM64_RCLOB {
                    if e.f.reg & bit(r as u32) != 0 {
                        ...
                        let restore_ins = Ins {
                            ...
                            arg: [Ref::Slot(StackSlot::from_signed(s2)), Ref::R],
                        };
            ...
            _ => {
                ...
                let cc = if s2_blk == next_bid {
                    std::mem::swap(&mut s1, &mut s2_blk);
                    c
                } else {
                    cc_neg(c)
                };
    ...
    if !apple {
        crate::codegen::emission::elf_emitfnfin(&f.name, e.out);
    }
}
