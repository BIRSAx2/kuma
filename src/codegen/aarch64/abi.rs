...
fn sttmps(tmp: &mut [Ref], cls: &[Cls], nreg: usize, mem: Ref, f: &mut Fn, buf: &mut InsBuffer) {
    ...
    let mut off: u64 = 0;
    for n in 0..nreg {
        tmp[n] = builder::newtmp("abi", cls[n], f);
        let r = builder::newtmp("abi", Cls::Kl, f);
        let sop = store_op[cls[n] as i8 as usize];
        buf.emit(sop, Cls::Kw, Ref::R, tmp[n], r);
        buf.emit(Op::Add, Cls::Kl, r, mem, builder::getcon(off as i64, f));
...
fn selret(bid: BlkId, f: &mut Fn, buf: &mut InsBuffer, typs: &[Typ]) {
    ...
    if j == Jmp::Retc {
        ...
        let cls = j
            .return_class()
            .expect("abi0 must normalize scalar return terminators");
        if cls.base() == 0 {
...
fn stkblob(r: Ref, c: &Class, f: &mut Fn, il: &mut Vec<Ins>, typs: &[Typ]) {
    ...
    let sz_ref = builder::getcon(sz as i64, f);
    let alloc_op = match al_idx {
        0 => Op::Alloc4,
        1 => Op::Alloc8,
        2 => Op::Alloc16,
        _ => unreachable!("unsupported aggregate alignment {al_idx}"),
    };
    il.push(Ins {
        op: alloc_op,
        cls: Cls::Kl,
...
fn selcall(
    ...
    for (idx, c) in ca.iter_mut().enumerate() {
        if c.class & CPTR != 0 {
            let new_tmp = builder::newtmp("abi", Cls::Kl, f);
    ...
    stk = align_up(stk, 16);
    let rstk = builder::getcon(stk as i64, f);
    ...
    if !i1.arg[1].is_none() {
        ...
        if cr.class & CPTR != 0 {
            ...
            for (n, &tmp_ref) in tmp.iter().enumerate().take(cr.nreg as usize) {
                let r = Ref::Tmp(TmpId(cr.reg[n] as u32));
                buf.emit(Op::Copy, cr.cls[n], tmp_ref, r, Ref::R);
            }
        }
    } else {
        if i1.cls.base() == 0 {
...
fn selpar(f: &mut Fn, par_ins: &[Ins], typs: &[Typ], apple: bool) -> (Vec<Ins>, Params) {
    ...
    if f.retty >= 0 {
        ...
        if cr.class & CPTR != 0 {
            f.retr = builder::newtmp("abi", Cls::Kl, f);
    ...
    for (idx, c) in ca.iter().enumerate() {
        ...
        if i.op == Op::Parc && (c.class & CPTR) == 0 {
            if c.class & CSTK != 0 {
                off = align_up(off, c.align);
                f.tmps[i.to.val() as usize].slot =
                    Some(StackSlot::from_signed(-((off + 2) as i32)));
                off += c.size;
            } else {
            ...
            let op = if i.op.is_parbh() {
                match i.op {
                    ...
                    Op::Parsh => Op::Loadsh,
                    Op::Paruh => Op::Loaduh,
                    _ => unreachable!("non-subword parameter in subword lowering"),
                }
            } else {
                Op::Load
            };
            buf.emit(
                ...
                c.cls[0],
                i.to,
                Ref::Slot(StackSlot::from_signed(-((off + 2) as i32))),
                Ref::R,
            );
            off += c.size;
        } else {
...
fn apple_selvaarg(f: &mut Fn, _bid: BlkId, i: &Ins, buf: &mut InsBuffer) {
    let c8 = builder::getcon(8, f);
    let ap = i.arg[0];
    let stk8 = builder::newtmp("abi", Cls::Kl, f);
    let stk = builder::newtmp("abi", Cls::Kl, f);
...
fn apple_selvastart(f: &mut Fn, p: &Params, ap: Ref, buf: &mut InsBuffer) {
    let off = builder::getcon(p.stk as i64, f);
    let stk = builder::newtmp("abi", Cls::Kl, f);
    let arg = builder::newtmp("abi", Cls::Kl, f);
    ...
    buf.emit(
        ...
        Cls::Kl,
        stk,
        Ref::Slot(StackSlot::from_signed(-1)),
        Ref::R,
    );
}
...
fn arm64_selvastart(f: &mut Fn, p: &Params, ap: Ref, buf: &mut InsBuffer) {
    let rsave = builder::newtmp("abi", Cls::Kl, f);

    let r0 = builder::newtmp("abi", Cls::Kl, f);
    ...
    buf.emit(
        ...
        r0,
        rsave,
        builder::getcon((p.stk + 192) as i64, f),
    );

    let r0 = builder::newtmp("abi", Cls::Kl, f);
    let r1 = builder::newtmp("abi", Cls::Kl, f);
    buf.emit(Op::Storel, Cls::Kw, Ref::R, r1, r0);
    buf.emit(Op::Add, Cls::Kl, r1, rsave, builder::getcon(64, f));
    buf.emit(Op::Add, Cls::Kl, r0, ap, builder::getcon(8, f));

    let r0 = builder::newtmp("abi", Cls::Kl, f);
    let r1 = builder::newtmp("abi", Cls::Kl, f);
    buf.emit(Op::Storel, Cls::Kw, Ref::R, r1, r0);
    buf.emit(Op::Add, Cls::Kl, r1, rsave, builder::getcon(192, f));
    buf.emit(
        Op::Addr,
        Cls::Kl,
        rsave,
    ...
    buf.emit(Op::Add, Cls::Kl, r0, ap, builder::getcon(16, f));
    ...
    buf.emit(
        ...
        Cls::Kw,
        Ref::R,
        builder::getcon(((p.ngp as i64) - 8) * 8, f),
        r0,
    );
    buf.emit(Op::Add, Cls::Kl, r0, ap, builder::getcon(24, f));
    ...
    buf.emit(
        ...
        Cls::Kw,
        Ref::R,
        builder::getcon(((p.nfp as i64) - 8) * 16, f),
        r0,
    );
    buf.emit(Op::Add, Cls::Kl, r0, ap, builder::getcon(28, f));
}
...
fn apple_extsb(f: &mut Fn) {
    ...
    for &bid in &rpo {
        ...
        let j = f.blks[bid.0 as usize].jmp.typ;
        if j.is_retbh() {
            let r = builder::newtmp("abi", Cls::Kw, f);
            let op = match j {
                Jmp::Retsb => Op::Extsb,
                Jmp::Retub => Op::Extub,
                Jmp::Retsh => Op::Extsh,
                Jmp::Retuh => Op::Extuh,
                _ => unreachable!("non-subword return in subword lowering"),
            };
        ...
        while idx > 0 {
            ...
            while arg_idx > arg_start {
                ...
                if ins[arg_idx].op.is_argbh() {
                    let new_to = builder::newtmp("abi", Cls::Kl, f);
                    let last = buf.last_mut();
                    last.to = new_to;
                }
            }
            for arg in &ins[arg_start..call_idx] {
                if arg.op.is_argbh() {
                    let op = match arg.op {
                        Op::Argsb => Op::Extsb,
                        Op::Argub => Op::Extub,
                        Op::Argsh => Op::Extsh,
                        Op::Arguh => Op::Extuh,
                        _ => unreachable!("non-subword argument in subword lowering"),
                    };
                    let ext_to = builder::newtmp("abi", Cls::Kw, f);
                    buf.emit(op, Cls::Kw, ext_to, arg.arg[0], Ref::R);
                }
            }
            idx = arg_start;
        }
