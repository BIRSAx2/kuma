    ...
    let ca = if j == Jmp::Retc {
        ...
        if aret.inmem != 0 {
            ...
            if aret.size > 8 {
                ...
                buf.emit(Op::Add, Cls::Kl, r, r0, builder::getcon(8, f));
            }
...
fn selcall(f: &mut Fn, call_seq: &[Ins], buf: &mut InsBuffer, ral: &mut Vec<Ins>, typs: &[Typ]) {
    ...
    if has_aret {
        if aret.inmem != 0 {
            hidden_ret = builder::newtmp("abi", Cls::Kl, f);
            ...
            if aret.size > 8 {
                let r = builder::newtmp("abi", Cls::Kl, f);
                aret.refs[1] = builder::newtmp("abi", aret.cls[1], f);
                buf.emit(Op::Storel, Cls::Kw, Ref::R, aret.refs[1], r);
                buf.emit(Op::Add, Cls::Kl, r, i1.to, builder::getcon(8, f));
            }
            aret.refs[0] = builder::newtmp("abi", aret.cls[0], f);
        ...
        ral.push(Ins {
            ...
            cls: Cls::Kl,
            to: hidden_ret,
            arg: [builder::getcon(aret.size as i64, f), Ref::R],
        });
    ...
    if !env.is_none() {
        ...
        buf.emit(
            ...
            Cls::Kw,
            preg(RAX),
            builder::getcon(((ca >> 8) & 15) as i64, f),
            Ref::R,
        );
    }
    ...
    for (i, a) in args.iter().zip(ac.iter()) {
        ...
        if i.op == Op::Argc {
            if a.size > 8 {
                ...
                buf.emit(Op::Add, Cls::Kl, r, i.arg[1], builder::getcon(8, f));
            }
    ...
    for (i, a) in args.iter().zip(ac.iter()) {
        ...
        buf.emit(Op::Add, Cls::Kl, r1, r, builder::getcon(off as i64, f));
        off += a.size;
    }
    buf.emit(
        Op::Salloc,
...
fn selpar(f: &mut Fn, ins: &[Ins], typs: &[Typ]) -> (Vec<Ins>, u32) {
    ...
    for (i, a) in ins.iter().zip(ac.iter_mut()) {
        ...
        if a.size > 8 {
            let r = builder::newtmp("abi", Cls::Kl, f);
            a.refs[1] = builder::newtmp("abi", Cls::Kl, f);
            buf.emit(Op::Storel, Cls::Kw, Ref::R, a.refs[1], r);
            buf.emit(Op::Add, Cls::Kl, r, i.to, builder::getcon(8, f));
        }
        a.refs[0] = builder::newtmp("abi", Cls::Kl, f);
        ...
        buf.emit(op, Cls::Kl, i.to, builder::getcon(a.size as i64, f), Ref::R);
    }
        ...
        match a.inmem {
            1 => {
                ...
                if let Ref::Tmp(tid) = i.to {
                    f.tmps[tid.0 as usize].slot = Some(StackSlot::from_signed(-s));
                }
                s += (a.size / 4) as i32;
                continue;
            }
...
fn expand_vastart_seq(f: &mut Fn, fa: u32, ap: Ref) -> Vec<Ins> {
    ...
    buf.emit(Op::Add, Cls::Kl, r1, preg(RBP), builder::getcon(-176, f));
    buf.emit(Op::Add, Cls::Kl, r0, ap, builder::getcon(16, f));

    let r0 = builder::newtmp("abi", Cls::Kl, f);
    let r1 = builder::newtmp("abi", Cls::Kl, f);
    buf.emit(Op::Storel, Cls::Kw, Ref::R, r1, r0);
    buf.emit(Op::Add, Cls::Kl, r1, preg(RBP), builder::getcon(sp, f));
    buf.emit(Op::Add, Cls::Kl, r0, ap, builder::getcon(8, f));

    let r0 = builder::newtmp("abi", Cls::Kl, f);
    buf.emit(Op::Storew, Cls::Kw, Ref::R, builder::getcon(fp, f), r0);
    buf.emit(Op::Add, Cls::Kl, r0, ap, builder::getcon(4, f));
    buf.emit(Op::Storew, Cls::Kw, Ref::R, builder::getcon(gp, f), ap);
...
fn lower_vaarg_block(f: &mut Fn, rpo_idx: usize, bid: BlkId, ins_idx: usize) {
    ...
    let loc = builder::newtmp("abi", Cls::Kl, f);
    let lreg = builder::newtmp("abi", Cls::Kl, f);
    let lstk = builder::newtmp("abi", Cls::Kl, f);
    let nr = builder::newtmp("abi", Cls::Kl, f);
    let c4 = builder::getcon(4, f);
    let c8 = builder::getcon(8, f);
    let c16 = builder::getcon(16, f);
    let limit = builder::getcon(if isint { 48 } else { 176 }, f);
    ...
    let r0 = builder::newtmp("abi", Cls::Kl, f);
    let r1 = builder::newtmp("abi", Cls::Kw, f);
    b.ins.push(Ins {
        op: Op::Add,
        cls: Cls::Kl,
        to: r0,
    ...
    let r0a = builder::newtmp("abi", Cls::Kl, f);
    let r1a = builder::newtmp("abi", Cls::Kl, f);
    let r0b = builder::newtmp("abi", Cls::Kw, f);
    let r1b = builder::newtmp("abi", Cls::Kl, f);
    breg.ins.push(Ins {
        op: Op::Add,
        cls: Cls::Kl,
        to: r0a,
    ...
    let r0c = builder::newtmp("abi", Cls::Kl, f);
    let r1c = builder::newtmp("abi", Cls::Kl, f);
    bstk.ins.push(Ins {
        op: Op::Add,
        cls: Cls::Kl,
        to: r0c,
