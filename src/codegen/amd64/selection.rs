...
fn slot_ref(s: i32) -> Ref {
    Ref::Slot(StackSlot::from_signed(s))
}

fn rslot(r: Ref, f: &Fn) -> i32 {
    match r {
        Ref::Tmp(id) if id.0 >= TMP0 => f.tmps[id.0 as usize].slot.map_or(-1, StackSlot::signed),
        _ => -1,
    }
}
...
fn float_const_ref(r: Ref, k: Cls, f: &mut Fn, apple: bool, literals: &mut FPStash) -> Ref {
    let Ref::Con(id) = r else { return r };
    let c = f.cons[id.0 as usize];
    let bytes = c.bits.i().to_le_bytes();
    let n = literals.stash(if k.is_wide() {
        &bytes[..8]
    } else {
        &bytes[..4]
    });
    ...
    if let Ref::Con(_) = r0 {
        if k.base() == 1 {
            ...
            buf.emit(
                ...
                k,
                r1,
                float_const_ref(r0, k, f, apple, literals),
                Ref::R,
            );
            *pr = r1;
            return;
...
fn fixarg_ins(
    ...
    fixarg_ref(&mut r, arg_use, f, buf, apple, literals);
...
fn selcmp(
    ...
    if matches!(arg[0], Ref::Con(_)) {
        ...
        fixarg_ins(
            buf,
            copy_idx,
            ArgUse::instruction(k, Op::Copy, 0),
            f,
            apple,
            literals,
        );
        ...
        fixarg_ins(
            buf,
            cmp_idx,
            ArgUse::instruction(k, Op::Xcmp, 1),
            f,
            apple,
            literals,
        );
    ...
    fixarg_ins(
        buf,
        cmp_idx,
        ArgUse::instruction(k, Op::Xcmp, 0),
        f,
        apple,
        literals,
    );
...
fn seljmp(bid: BlkId, f: &mut Fn, buf: &mut InsBuffer, apple: bool, literals: &mut FPStash) {
    ...
    selcmp(&mut arg, Cls::Kw, false, f, buf, apple, literals);
...
fn sel(mut i: Ins, f: &mut Fn, buf: &mut InsBuffer, apple: bool, literals: &mut FPStash) {
    if let Ref::Tmp(tid) = i.to
        && tid.0 >= TMP0
        && !builder::isreg(i.to)
        && !builder::isreg(i.arg[0])
        && !builder::isreg(i.arg[1])
        && f.tmps[tid.0 as usize].nuse == 0
    {
        builder::chuse(i.arg[0], -1, f);
        builder::chuse(i.arg[1], -1, f);
        return;
    }
    ...
    match i.op {
        ...
        Op::Sar | Op::Shr | Op::Shl if !matches!(i.arg[1], Ref::Con(_)) => {
            ...
            fixarg_ins(
                buf,
                op_idx,
                ArgUse::instruction(crate::ir::internal::argcls(&i, 0), i.op, 0),
                f,
                apple,
                literals,
            );
        ...
        Op::Uwtof => {
            let r0 = builder::newtmp("utof", Cls::Kl, f);
        ...
        Op::Ultof => {
            ...
            for t in tmp.iter_mut().take(4) {
                *t = builder::newtmp("utof", Cls::Kl, f);
            }
            for t in tmp.iter_mut().skip(4) {
                *t = builder::newtmp("utof", kc, f);
            }
            let r0 = builder::newtmp("utof", k, f);
            buf.emit(Op::Cast, k, i.to, tmp[6], Ref::R);
            buf.emit(Op::Add, kc, tmp[6], tmp[4], tmp[5]);
            buf.emit(Op::Shl, kc, tmp[5], tmp[1], builder::getcon(sh, f));
            ...
            buf.emit(Op::Shr, Cls::Kl, tmp[1], i.arg[0], builder::getcon(63, f));
            let idx = buf.len() - 1;
            fixarg_ins(
                buf,
                idx,
                ArgUse::instruction(Cls::Kl, Op::Shr, 0),
                f,
                apple,
                literals,
            );
            buf.emit(Op::And, Cls::Kl, tmp[0], i.arg[0], builder::getcon(1, f));
            let idx = buf.len() - 1;
            fixarg_ins(
                buf,
                idx,
                ArgUse::instruction(Cls::Kl, Op::And, 0),
                f,
                apple,
                literals,
            );
        ...
        Op::Stoui | Op::Dtoui => {
            let (mut op, kc, addc) = if i.op == Op::Stoui {
                (Op::Stosi, Cls::Ks, builder::getcon(0xdf000000u32 as i64, f))
            } else {
                (
                    Op::Dtosi,
                    Cls::Kd,
                    builder::getcon(0xc3e0000000000000u64 as i64, f),
                )
            };
            if k == Cls::Kw {
                let r0 = builder::newtmp("ftou", Cls::Kl, f);
                ...
                let idx = buf.len() - 1;
                fixarg_ins(buf, idx, ArgUse::instruction(kc, op, 0), f, apple, literals);
                return;
            }
            let r0 = builder::newtmp("ftou", kc, f);
            let mut tmp = [Ref::R; 4];
            for t in &mut tmp {
                *t = builder::newtmp("ftou", Cls::Kl, f);
            }
            ...
            fixarg_ins(
                buf,
                idx,
                ArgUse::instruction(kc, Op::Add, 0),
                f,
                apple,
                literals,
            );
            fixarg_ins(
                buf,
                idx,
                ArgUse::instruction(kc, Op::Add, 1),
                f,
                apple,
                literals,
            );
            buf.emit(Op::Sar, Cls::Kl, tmp[1], tmp[0], builder::getcon(63, f));
            buf.emit(op, Cls::Kl, tmp[0], i.arg[0], Ref::R);
            let idx = buf.len() - 1;
            fixarg_ins(
                buf,
                idx,
                ArgUse::instruction(Cls::Kl, op, 0),
                f,
                apple,
                literals,
            );
        ...
        Op::Stored | Op::Stores => {
            ...
            fixarg_ins(
                buf,
                idx,
                ArgUse::instruction(crate::ir::internal::argcls(&i, 1), i.op, 1),
                f,
                apple,
                literals,
            );
        ...
        _ => {
            if let Some((mut kind, kc)) = builder::iscmp(i.op) {
                let cmp_cls = Cls::from_i8(kc as i8);
                match kind {
                    x if x == N_CMP_I as i32 + CmpF::Cfeq as i32 => {
                        let r0 = builder::newtmp("isel", Cls::Kw, f);
                        let r1 = builder::newtmp("isel", Cls::Kw, f);
                ...
                selcmp(&mut i.arg, cmp_cls, swap, f, buf, apple, literals);
                return;
            }
...
pub fn isel(f: &mut Fn, t: &Target, literals: &mut FPStash) {
    ...
    for &bid in &rpo {
            ...
            for pi in 0..f.blks[succ.0 as usize].phi.len() {
                ...
                for (ai, pbid) in blks.iter().enumerate() {
                    if *pbid == bid {
                        ...
                        fixarg_ref(&mut r, ArgUse::phi(cls), f, &mut buf, apple, literals);
        ...
        for ins in ins.into_iter().rev() {
            sel(ins, f, &mut buf, apple, literals);
        }
