...
//! Shared assembly and IR emission.

use std::fmt::Write as _;

use crate::ir::internal::{
    Cls, Con, ConType, Dat, DatItem, Fn, Jmp, Lnk, OP_TABLE, Op, Ref, SymType, TMP0, Target, Typ,
};

/// Floating-point bit patterns awaiting literal-data emission.
pub struct FPStash {
...
impl FPStash {
    ...
    /// Store a 4-, 8-, or 16-byte pattern and return its deduplicated index.
...
/// Assembly-emission state owned by one compilation session.
pub(crate) struct EmissionState {
    pub(crate) floating_literals: FPStash,
    pub(crate) debug_files: DbgFileState,
    pub(crate) next_block_label: u32,
}

impl EmissionState {
    pub(crate) fn new() -> Self {
        Self {
            floating_literals: FPStash::new(),
            debug_files: DbgFileState::new(),
            next_block_label: 0,
        }
    }
}

/// State shared by the items in one data definition.
pub struct DatState {
...
fn emitlnk(name: &str, lnk: &Lnk, sec: Sec, target: Option<&Target>, out: &mut String) {
    let is_apple = target.is_some_and(|t| t.apple);
...
/// Commit a pending data section and emit leading zeros.
...
/// Print a reference, resolving type names from `typs`.
...
/// Return the IR character for a value class.
    ...
    for (order_idx, &bi) in order.iter().enumerate() {
        ...
        match jmp.typ {
            ...
            | Jmp::Ret0 => {
                ...
                if jmp.typ == Jmp::Retc && f.retty >= 0 && (f.retty as usize) < typs.len() {
